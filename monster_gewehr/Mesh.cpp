...
void CStandardMesh::LoadMeshFromFile(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, FILE *pInFile, int TileX, int TileY)
{
	char pstrToken[64] = { '\0' };
	...
	for ( ; ; )
	{
		...
		else if (!strcmp(pstrToken, "<TextureCoords0>:"))
		{
			...
			if (nTextureCoords > 0)
			{
				...
				{
					m_pxmf2TextureCoords0[i].x *= TileX;
					m_pxmf2TextureCoords0[i].y *= TileY;
				}
...
void CSkinnedMesh::PrepareSkinning(GameObjectModel *pModelRootObject)
{
	for (int j = 0; j < m_nSkinningBones; j++)
	{
	...
	for ( ; ; )
	{
		...
		else if (!strcmp(pstrToken, "<BoneNames>:"))
		{
			...
			if (m_nSkinningBones > 0) 
			{
				...
				m_ppSkinningBoneFrameCaches = new GameObjectModel*[m_nSkinningBones];
...
CBoxMesh::CBoxMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, float fWidth, float fHeight, float fDepth, float cx, float cy, float cz) : CMesh(pd3dDevice, pd3dCommandList)
{
	m_nVertices = 36;
	m_d3dPrimitiveTopology = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
	...
	m_pxmf3Positions[0] = XMFLOAT3(cx - fx, cy + fy, cz + fz);
	m_pxmf3Positions[1] = XMFLOAT3(cx - fx, cy - fy, cz + fz);
	m_pxmf3Positions[2] = XMFLOAT3(cx + fx, cy + fy, cz + fz);
	m_pxmf3Positions[3] = XMFLOAT3(cx - fx, cy - fy, cz + fz);
	m_pxmf3Positions[4] = XMFLOAT3(cx + fx, cy - fy, cz + fz);
	m_pxmf3Positions[5] = XMFLOAT3(cx + fx, cy + fy, cz + fz);
	// Back Quad										
	m_pxmf3Positions[6] = XMFLOAT3(cx + fx, cy + fy, cz - fz);
	m_pxmf3Positions[7] = XMFLOAT3(cx + fx, cy - fy, cz - fz);
	m_pxmf3Positions[8] = XMFLOAT3(cx - fx, cy + fy, cz - fz);
	m_pxmf3Positions[9] = XMFLOAT3(cx + fx, cy - fy, cz - fz);
	m_pxmf3Positions[10] = XMFLOAT3(cx - fx, cy - fy, cz - fz);
	m_pxmf3Positions[11] = XMFLOAT3(cx - fx, cy + fy, cz - fz);
	// Left Quad										
	m_pxmf3Positions[12] = XMFLOAT3(cx - fx, cy + fy, cz - fz);
	m_pxmf3Positions[13] = XMFLOAT3(cx - fx, cy - fy, cz - fz);
	m_pxmf3Positions[14] = XMFLOAT3(cx - fx, cy + fy, cz + fz);
	m_pxmf3Positions[15] = XMFLOAT3(cx - fx, cy - fy, cz - fz);
	m_pxmf3Positions[16] = XMFLOAT3(cx - fx, cy - fy, cz + fz);
	m_pxmf3Positions[17] = XMFLOAT3(cx - fx, cy + fy, cz + fz);
	// Right Quad										
	m_pxmf3Positions[18] = XMFLOAT3(cx + fx, cy + fy, cz + fz);
	m_pxmf3Positions[19] = XMFLOAT3(cx + fx, cy - fy, cz + fz);
	m_pxmf3Positions[20] = XMFLOAT3(cx + fx, cy + fy, cz - fz);
	m_pxmf3Positions[21] = XMFLOAT3(cx + fx, cy - fy, cz + fz);
	m_pxmf3Positions[22] = XMFLOAT3(cx + fx, cy - fy, cz - fz);
	m_pxmf3Positions[23] = XMFLOAT3(cx + fx, cy + fy, cz - fz);
	// Top Quad											
	m_pxmf3Positions[24] = XMFLOAT3(cx - fx, cy + fy, cz - fz);
	m_pxmf3Positions[25] = XMFLOAT3(cx - fx, cy + fy, cz + fz);
	m_pxmf3Positions[26] = XMFLOAT3(cx + fx, cy + fy, cz - fz);
	m_pxmf3Positions[27] = XMFLOAT3(cx - fx, cy + fy, cz + fz);
	m_pxmf3Positions[28] = XMFLOAT3(cx + fx, cy + fy, cz + fz);
	m_pxmf3Positions[29] = XMFLOAT3(cx + fx, cy + fy, cz - fz);
	// Bottom Quad										
	m_pxmf3Positions[30] = XMFLOAT3(cx - fx, cy - fy, cz + fz);
	m_pxmf3Positions[31] = XMFLOAT3(cx - fx, cy - fy, cz - fz);
	m_pxmf3Positions[32] = XMFLOAT3(cx + fx, cy - fy, cz + fz);
	m_pxmf3Positions[33] = XMFLOAT3(cx - fx, cy - fy, cz - fz);
	m_pxmf3Positions[34] = XMFLOAT3(cx + fx, cy - fy, cz - fz);
	m_pxmf3Positions[35] = XMFLOAT3(cx + fx, cy - fy, cz + fz);
...
CBoxMesh::CBoxMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, BoundingOrientedBox* box) : CMesh(pd3dDevice, pd3dCommandList)
{
	m_nVertices = 36;
	m_d3dPrimitiveTopology = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
	...
	m_pxmf3Positions[0] = corners[0]; // (-x, +y, +z)
	m_pxmf3Positions[1] = corners[1]; // (-x, -y, +z)
	m_pxmf3Positions[2] = corners[2]; // (+x, +y, +z)
	m_pxmf3Positions[3] = corners[1]; // (-x, -y, +z)
	m_pxmf3Positions[4] = corners[3]; // (+x, -y, +z)
	m_pxmf3Positions[5] = corners[2]; // (+x, +y, +z)

	// Back Quad
	m_pxmf3Positions[6] = corners[4]; // (+x, +y, -z)
	m_pxmf3Positions[7] = corners[7]; // (+x, -y, -z)
	m_pxmf3Positions[8] = corners[6]; // (-x, +y, -z)
	m_pxmf3Positions[9] = corners[7]; // (+x, -y, -z)
	m_pxmf3Positions[10] = corners[5]; // (-x, -y, -z)
	m_pxmf3Positions[11] = corners[6]; // (-x, +y, -z)

	// Left Quad
	m_pxmf3Positions[12] = corners[6]; // (-x, +y, -z)
	m_pxmf3Positions[13] = corners[5]; // (-x, -y, -z)
	m_pxmf3Positions[14] = corners[0]; // (-x, +y, +z)
	m_pxmf3Positions[15] = corners[5]; // (-x, -y, -z)
	m_pxmf3Positions[16] = corners[1]; // (-x, -y, +z)
	m_pxmf3Positions[17] = corners[0]; // (-x, +y, +z)

	// Right Quad
	m_pxmf3Positions[18] = corners[2]; // (+x, +y, +z)
	m_pxmf3Positions[19] = corners[3]; // (+x, -y, +z)
	m_pxmf3Positions[20] = corners[4]; // (+x, +y, -z)
	m_pxmf3Positions[21] = corners[3]; // (+x, -y, +z)
	m_pxmf3Positions[22] = corners[7]; // (+x, -y, -z)
	m_pxmf3Positions[23] = corners[4]; // (+x, +y, -z)

	// Top Quad
	m_pxmf3Positions[24] = corners[6]; // (-x, +y, -z)
	m_pxmf3Positions[25] = corners[0]; // (-x, +y, +z)
	m_pxmf3Positions[26] = corners[4]; // (+x, +y, -z)
	m_pxmf3Positions[27] = corners[0]; // (-x, +y, +z)
	m_pxmf3Positions[28] = corners[2]; // (+x, +y, +z)
	m_pxmf3Positions[29] = corners[4]; // (+x, +y, -z)

	// Bottom Quad
	m_pxmf3Positions[30] = corners[1]; // (-x, -y, +z)
	m_pxmf3Positions[31] = corners[5]; // (-x, -y, -z)
	m_pxmf3Positions[32] = corners[3]; // (+x, -y, +z)
	m_pxmf3Positions[33] = corners[5]; // (-x, -y, -z)
	m_pxmf3Positions[34] = corners[7]; // (+x, -y, -z)
	m_pxmf3Positions[35] = corners[3]; // (+x, -y, +z)
...
{
	{
		...
		//요기에 저장
		float fx = fWidth * 1.0f, fy = fHeight * 1.0f, fz = fDepth * 1.0f;
		m_pxmf3Positions[0] = XMFLOAT3(-fx, +fy, 0.f);
		m_pxmf3Positions[1] = XMFLOAT3(-fx, -fy, 0.f);
		m_pxmf3Positions[2] = XMFLOAT3(+fx, +fy, 0.f);
		m_pxmf3Positions[3] = XMFLOAT3(-fx, -fy, 0.f);
		m_pxmf3Positions[4] = XMFLOAT3(+fx, -fy, 0.f);
		m_pxmf3Positions[5] = XMFLOAT3(+fx, +fy, 0.f);
		...
		m_pxmf2TextureCoords1 = new XMFLOAT2[m_nVertices];
		//요기에 저장
		m_pxmf2TextureCoords1[0] = XMFLOAT2(0.0f, 0.0f);
		m_pxmf2TextureCoords1[1] = XMFLOAT2(0.0f, 1.0f);
		m_pxmf2TextureCoords1[2] = XMFLOAT2(1.0f, 0.0f);
		m_pxmf2TextureCoords1[3] = XMFLOAT2(0.0f, 1.0f);
		m_pxmf2TextureCoords1[4] = XMFLOAT2(1.0f, 1.0f);
		m_pxmf2TextureCoords1[5] = XMFLOAT2(1.0f, 0.0f);
