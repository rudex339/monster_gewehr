...
RenderSystem::RenderSystem(ID3D12GraphicsCommandList* commandList)
{
...
void RenderSystem::CreateGraphicsRootSignature(ID3D12Device* pd3dDevice)
{
	...
	pd3dDevice->CreateRootSignature(0, pd3dSignatureBlob->GetBufferPointer(), pd3dSignatureBlob->GetBufferSize(), __uuidof(ID3D12RootSignature), (void**)mRootSignature.Get());
...
{
	if (p_object->ShaderTag != cur_Shader) {
		//쉐이더 교체
		m_ShaderList[p_object->ShaderTag].get()->prepareShader();
		//
